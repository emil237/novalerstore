#!/bin/sh
#
###command=wget -q "--no-check-certificate" https://raw.githubusercontent.com/emil237/novalerstore/main/suptv.sh -O - | /bin/sh
###########################
PYTHON_VERSION=$(python -c"import platform; print(platform.python_version())")
##########################
echo " remove old plugins "
if [ -d /usr/lib/enigma2/python/Plugins/Extensions/suptv ]; then
echo "> removing package please wait..."
sleep 1;
rm -rf /usr/lib/enigma2/python/Plugins/Extensions/suptv > /dev/null 2>&1
fi
...
cd /tmp
set -e
wget --show-progress "$url/suptv-py3.11-6.0-r0.ipk"
sleep 1;
opkg install --force-reinstall suptv-py3.11-6.0-r0.ipk
set +e
cd ..
wait
rm -f /tmp/suptv-py3.11-6.0-r0.ipk
...
cd /tmp
set -e
wget --show-progress "$url/suptv-py3.12-6.0-r0.ipk"
sleep 1;
opkg install --force-reinstall suptv-py3.12-6.0-r0.ipk
set +e
cd ..
wait
rm -f /tmp/suptv-py3.12-6.0-r0.ipk

elif [ "$PYTHON_VERSION" == 3.9.9 -o "$PYTHON_VERSION" == 3.9.7 -o "$PYTHON_VERSION" == 3.9.6 -o "$PYTHON_VERSION" == 3.8.5 ]; then
echo ":You have $PYTHON_VERSION image ..."
cd /tmp
set -e
wget --show-progress "$url/suptv-py3.9-6.0-r0.ipk"
sleep 1;
opkg install --force-reinstall suptv-py3.9-6.0-r0.ipk
set +e
cd ..
wait
rm -f /tmp/suptv-py3.9-6.0-r0.ipk

elif [ "$PYTHON_VERSION" == 2.7.18 ]; then
echo ":You have $PYTHON_VERSION image ..."
cd /tmp
set -e
wget --show-progress "$url/suptv-py2-6.0-r0.ipk"
sleep 1;
opkg install --force-reinstall suptv-py2-6.0-r0.ipk
set +e
cd ..
wait
rm -f /tmp/suptv-py2-6.0-r0.ipk
