#!/bin/sh
#
###command=wget -q "--no-check-certificate" https://raw.githubusercontent.com/emil237/novalerstore/main/ultracam.sh -O - | /bin/sh
###########################
...
cd /tmp
set -e
wget --show-progress "$url/ultracam-py3.11-3.0-r0.ipk"
sleep 1;
opkg install --force-reinstall ultracam-py3.11-3.0-r0.ipk
set +e
cd ..
wait
rm -f /tmp/ultracam-py3.11-3.0-r0.ipk
...
cd /tmp
set -e
wget --show-progress "$url/ultracam-py3.12-3.0-r0.ipk"
sleep 1;
opkg install --force-reinstall ultracam-py3.12-3.0-r0.ipk
set +e
cd ..
wait
rm -f /tmp/ultracam-py3.12-3.0-r0.ipk

elif [ "$PYTHON_VERSION" == 3.9.9 -o "$PYTHON_VERSION" == 3.9.7 -o "$PYTHON_VERSION" == 3.9.6 -o "$PYTHON_VERSION" == 3.8.5 ]; then
echo ":You have $PYTHON_VERSION image ..."
cd /tmp
set -e
wget --show-progress "$url/ultracam-py3.9-3.0-r0.ipk"
sleep 1;
opkg install --force-reinstall ultracam-py3.9-3.0-r0.ipk
set +e
cd ..
wait
rm -f /tmp/ultracam-py3.9-3.0-r0.ipk

elif [ "$PYTHON_VERSION" == 2.7.18 ]; then
echo ":You have $PYTHON_VERSION image ..."
cd /tmp
set -e
wget --show-progress "$url/ultracam-py2-3.0-r0.ipk"
sleep 1;
opkg install --force-reinstall ultracam-py2-3.0-r0.ipk
set +e
cd ..
wait
rm -f /tmp/ultracam-py2-3.0-r0.ipk
