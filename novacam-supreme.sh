#!/bin/sh
#
###command=wget -q "--no-check-certificate" https://raw.githubusercontent.com/emil237/novalerstore/main/novacam-supreme.sh -O - | /bin/sh
###########################
PYTHON_VERSION=$(python -c"import platform; print(platform.python_version())")
##########################
echo " remove old plugins "
if [ -d /usr/lib/enigma2/python/Plugins/Extensions/NOVACAMSupreme ]; then
echo "> removing package please wait..."
sleep 1;
rm -rf /usr/lib/enigma2/python/Plugins/Extensions/NOVACAMSupreme > /dev/null 2>&1
fi
status='/var/lib/opkg/status'
package='enigma2-plugin-extensions-novacam-supreme'
...
cd /tmp
set -e
wget --show-progress "$url/novacam-supreme-py3.11-9.2-r0.ipk"
sleep 1;
opkg install --force-reinstall novacam-supreme-py3.11-9.2-r0.ipk
set +e
cd ..
wait
rm -f /tmp/novacam-supreme-py3.11-9.2-r0.ipk
...
cd /tmp
set -e
wget --show-progress "$url/novacam-supreme-py3.12-9.2-r0.ipk"
sleep 1;
opkg install --force-reinstall novacam-supreme-py3.12-9.2-r0.ipk
set +e
cd ..
wait
rm -f /tmp/novacam-supreme-py3.12-9.2-r0.ipk

elif [ "$PYTHON_VERSION" == 3.9.9 -o "$PYTHON_VERSION" == 3.9.7 -o "$PYTHON_VERSION" == 3.9.6 -o "$PYTHON_VERSION" == 3.8.5 ]; then
echo ":You have $PYTHON_VERSION image ..."
cd /tmp
set -e
wget --show-progress "$url/novacam-supreme-py3.9-9.2-r0.ipk"
sleep 1;
opkg install --force-reinstall novacam-supreme-py3.9-9.2-r0.ipk
set +e
cd ..
wait
rm -f /tmp/novacam-supreme-py3.9-9.2-r0.ipk
